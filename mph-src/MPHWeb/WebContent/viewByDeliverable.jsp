	...
	<div id="content">
		<div id="mainContent">
			...
			<h4 align="left">Nome Progetto: <form method="post" id="logoutforum<%=p.getIdProject()%>" action="ServletProject?to=toProjectPage&idProj=<%=p.getIdProject()%>">
	                        <u><a onclick="javascript:document.getElementById('logoutforum<%=p.getIdProject()%>').submit();"><%=p.getProjectName()%></a></u><br /></form>
			<br>
			<%
				if (dels != null) {
			%>
									...
									for (File f : files) {
										...
										if (del.getIdDeliverable() == d
												...
												&& gr.getIdGroup() == g
														.getIdGroup()) {
			%>
			<a href="./ServletUpDown?to=downloadSingle&idFile=<%=f.getIdFile()%>"
			...
			<%
									...
									<form method="post" id="getZip<%=g.getIdGroup() %>"  action="ServletUpDown?to=getZip&idDel=<%=d.getIdDeliverable()%>">
									<u><a onclick="javascript:document.getElementById('getZip<%=g.getIdGroup() %>').submit();">get ZipFile By Deliverable</a></u>
									</form>
								<%}
			%>
